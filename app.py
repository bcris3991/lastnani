...
# ─── ML IMPORT ────────────────────────────────────────────────────────────────
from ml_predictor import predict_demand, get_category_summary, predict_low_stock
    ...
    with get_db() as db:
        db.executescript('''
            ...
            CREATE TABLE IF NOT EXISTS items (
                ...
                created_at TEXT DEFAULT (datetime('now')),
                added_by INTEGER,
                added_by_name TEXT DEFAULT 'System',
        ...
        if count == 0:
            ...
            for i in items:
                ...
                db.execute("INSERT INTO items (item_name, description, category, quantity, status, added_by_name, initial_quantity) VALUES (?,?,?,?,?,?,?)",
                    (i[0], i[1], i[2], i[3], status, 'System', i[3]))
        db.commit()
        ...
        for col, definition in [
            ...
            try:
                db.execute(f"ALTER TABLE items ADD COLUMN {col} {definition}")
                db.commit()
            except Exception:
                pass
...
def add_item():
    ...
    db = get_db()
    cur = db.execute(
        "INSERT INTO items (item_name, description, category, quantity, status, added_by, added_by_name, initial_quantity) VALUES (?,?,?,?,?,?,?,?)",
        (name, desc, category, qty, status, session['user_id'], session['name'], qty))
    item_id = cur.lastrowid
    db.execute(
        "INSERT INTO item_logs (item_id, action, quantity_change, quantity_after, done_by_id, done_by_name, notes) VALUES (?,?,?,?,?,?,?)",
        (item_id, 'Added', qty, qty, session['user_id'], session['name'], f'Initial stock added: {qty} unit(s)'))
    db.commit()
...
@app.route('/items/log/<int:item_id>')
@login_required
@role_required('Staff', 'Admin')
def item_log(item_id):
    ...
    if not item:
        ...
        return redirect(url_for('items'))
    logs = db.execute(
        "SELECT * FROM item_logs WHERE item_id=? ORDER BY created_at DESC", (item_id,)
    ).fetchall()
    notifs = db.execute("SELECT COUNT(*) as c FROM notifications WHERE user_id=? AND is_read=0", (session['user_id'],)).fetchone()['c']
    return render_template('item_log.html', item=item, logs=logs, notif_count=notifs)
...
def borrow_requests():
    ...
    if role == 'Student':
        ...
        query = """SELECT br.*, u.name as borrower, u.email, i.item_name, i.category,
                          i.added_by_name, i.created_at as item_created_at, i.initial_quantity
...
def approve_request(req_id):
    ...
    if req and req['status'] == 'Pending':
        ...
        if item['quantity'] >= req['quantity_requested']:
            ...
            db.execute(
                "INSERT INTO item_logs (item_id, action, quantity_change, quantity_after, done_by_id, done_by_name, notes) VALUES (?,?,?,?,?,?,?)",
                (req['item_id'], 'Borrowed', -req['quantity_requested'], new_qty,
                 session['user_id'], session['name'],
                 f'Borrowed by user #{req["user_id"]} (Request #{req_id})'))
            db.commit()
...
def return_item(req_id):
    ...
    if req and req['status'] == 'Approved' and (session['role'] in ('Staff','Admin') or req['user_id'] == session['user_id']):
        ...
        db.execute(
            "INSERT INTO item_logs (item_id, action, quantity_change, quantity_after, done_by_id, done_by_name, notes) VALUES (?,?,?,?,?,?,?)",
            (req['item_id'], 'Returned', req['quantity_requested'], new_qty,
             req['user_id'], session['name'],
             f'Returned by user #{req["user_id"]} (Request #{req_id})'))
        db.commit()
...
def predictions():
    ...
    low_stock_preds = predict_low_stock(DATABASE)
    ...
    return render_template(
        ...
        low_stock_predictions=low_stock_preds,
        notif_count=notifs,
